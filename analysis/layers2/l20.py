import numpy as np




# Generated from reverse engineering
def l20_g(x: np.ndarray) -> np.ndarray:
    # x is a list (or vector) of length 212
    out = np.empty(212, dtype=np.float32)
    
    # for i in range(0, 140):
    for i in range(0, 140):
        s = x[0 + i]
        out[0 + i] = s if s > 0 else 0.0 # ReLu
        
    # biases: 0xf (len=4)
    biases = [1.0, 1.0, 1.0, 1.0]
    # for i in range(140, 144):
    for i in range(0, 4):
        s = -1 * x[140 + i]
        s += biases[i]
        out[140 + i] = s if s > 0 else 0.0 # ReLu
        
    # for i in range(144, 148):
    for i in range(0, 4):
        s = x[144 + i] + -16.0 * x[i + 148] + 16.0 * x[i + 152]
        out[144 + i] = s if s > 0 else 0.0 # ReLu
        
    # for i in range(148, 212):
    for i in range(0, 64):
        s = x[156 + i]
        out[148 + i] = s if s > 0 else 0.0 # ReLu
        
    return out




def l20_0(x):
    # x is a list (or vector) of length 220
    return max(0, x[0])
def l20_1(x):
    # x is a list (or vector) of length 220
    return max(0, x[1])
def l20_2(x):
    # x is a list (or vector) of length 220
    return max(0, x[2])
def l20_3(x):
    # x is a list (or vector) of length 220
    return max(0, x[3])
def l20_4(x):
    # x is a list (or vector) of length 220
    return max(0, x[4])
def l20_5(x):
    # x is a list (or vector) of length 220
    return max(0, x[5])
def l20_6(x):
    # x is a list (or vector) of length 220
    return max(0, x[6])
def l20_7(x):
    # x is a list (or vector) of length 220
    return max(0, x[7])
def l20_8(x):
    # x is a list (or vector) of length 220
    return max(0, x[8])
def l20_9(x):
    # x is a list (or vector) of length 220
    return max(0, x[9])
def l20_10(x):
    # x is a list (or vector) of length 220
    return max(0, x[10])
def l20_11(x):
    # x is a list (or vector) of length 220
    return max(0, x[11])
def l20_12(x):
    # x is a list (or vector) of length 220
    return max(0, x[12])
def l20_13(x):
    # x is a list (or vector) of length 220
    return max(0, x[13])
def l20_14(x):
    # x is a list (or vector) of length 220
    return max(0, x[14])
def l20_15(x):
    # x is a list (or vector) of length 220
    return max(0, x[15])
def l20_16(x):
    # x is a list (or vector) of length 220
    return max(0, x[16])
def l20_17(x):
    # x is a list (or vector) of length 220
    return max(0, x[17])
def l20_18(x):
    # x is a list (or vector) of length 220
    return max(0, x[18])
def l20_19(x):
    # x is a list (or vector) of length 220
    return max(0, x[19])
def l20_20(x):
    # x is a list (or vector) of length 220
    return max(0, x[20])
def l20_21(x):
    # x is a list (or vector) of length 220
    return max(0, x[21])
def l20_22(x):
    # x is a list (or vector) of length 220
    return max(0, x[22])
def l20_23(x):
    # x is a list (or vector) of length 220
    return max(0, x[23])
def l20_24(x):
    # x is a list (or vector) of length 220
    return max(0, x[24])
def l20_25(x):
    # x is a list (or vector) of length 220
    return max(0, x[25])
def l20_26(x):
    # x is a list (or vector) of length 220
    return max(0, x[26])
def l20_27(x):
    # x is a list (or vector) of length 220
    return max(0, x[27])
def l20_28(x):
    # x is a list (or vector) of length 220
    return max(0, x[28])
def l20_29(x):
    # x is a list (or vector) of length 220
    return max(0, x[29])
def l20_30(x):
    # x is a list (or vector) of length 220
    return max(0, x[30])
def l20_31(x):
    # x is a list (or vector) of length 220
    return max(0, x[31])
def l20_32(x):
    # x is a list (or vector) of length 220
    return max(0, x[32])
def l20_33(x):
    # x is a list (or vector) of length 220
    return max(0, x[33])
def l20_34(x):
    # x is a list (or vector) of length 220
    return max(0, x[34])
def l20_35(x):
    # x is a list (or vector) of length 220
    return max(0, x[35])
def l20_36(x):
    # x is a list (or vector) of length 220
    return max(0, x[36])
def l20_37(x):
    # x is a list (or vector) of length 220
    return max(0, x[37])
def l20_38(x):
    # x is a list (or vector) of length 220
    return max(0, x[38])
def l20_39(x):
    # x is a list (or vector) of length 220
    return max(0, x[39])
def l20_40(x):
    # x is a list (or vector) of length 220
    return max(0, x[40])
def l20_41(x):
    # x is a list (or vector) of length 220
    return max(0, x[41])
def l20_42(x):
    # x is a list (or vector) of length 220
    return max(0, x[42])
def l20_43(x):
    # x is a list (or vector) of length 220
    return max(0, x[43])
def l20_44(x):
    # x is a list (or vector) of length 220
    return max(0, x[44])
def l20_45(x):
    # x is a list (or vector) of length 220
    return max(0, x[45])
def l20_46(x):
    # x is a list (or vector) of length 220
    return max(0, x[46])
def l20_47(x):
    # x is a list (or vector) of length 220
    return max(0, x[47])
def l20_48(x):
    # x is a list (or vector) of length 220
    return max(0, x[48])
def l20_49(x):
    # x is a list (or vector) of length 220
    return max(0, x[49])
def l20_50(x):
    # x is a list (or vector) of length 220
    return max(0, x[50])
def l20_51(x):
    # x is a list (or vector) of length 220
    return max(0, x[51])
def l20_52(x):
    # x is a list (or vector) of length 220
    return max(0, x[52])
def l20_53(x):
    # x is a list (or vector) of length 220
    return max(0, x[53])
def l20_54(x):
    # x is a list (or vector) of length 220
    return max(0, x[54])
def l20_55(x):
    # x is a list (or vector) of length 220
    return max(0, x[55])
def l20_56(x):
    # x is a list (or vector) of length 220
    return max(0, x[56])
def l20_57(x):
    # x is a list (or vector) of length 220
    return max(0, x[57])
def l20_58(x):
    # x is a list (or vector) of length 220
    return max(0, x[58])
def l20_59(x):
    # x is a list (or vector) of length 220
    return max(0, x[59])
def l20_60(x):
    # x is a list (or vector) of length 220
    return max(0, x[60])
def l20_61(x):
    # x is a list (or vector) of length 220
    return max(0, x[61])
def l20_62(x):
    # x is a list (or vector) of length 220
    return max(0, x[62])
def l20_63(x):
    # x is a list (or vector) of length 220
    return max(0, x[63])
def l20_64(x):
    # x is a list (or vector) of length 220
    return max(0, x[64])
def l20_65(x):
    # x is a list (or vector) of length 220
    return max(0, x[65])
def l20_66(x):
    # x is a list (or vector) of length 220
    return max(0, x[66])
def l20_67(x):
    # x is a list (or vector) of length 220
    return max(0, x[67])
def l20_68(x):
    # x is a list (or vector) of length 220
    return max(0, x[68])
def l20_69(x):
    # x is a list (or vector) of length 220
    return max(0, x[69])
def l20_70(x):
    # x is a list (or vector) of length 220
    return max(0, x[70])
def l20_71(x):
    # x is a list (or vector) of length 220
    return max(0, x[71])
def l20_72(x):
    # x is a list (or vector) of length 220
    return max(0, x[72])
def l20_73(x):
    # x is a list (or vector) of length 220
    return max(0, x[73])
def l20_74(x):
    # x is a list (or vector) of length 220
    return max(0, x[74])
def l20_75(x):
    # x is a list (or vector) of length 220
    return max(0, x[75])
def l20_76(x):
    # x is a list (or vector) of length 220
    return max(0, x[76])
def l20_77(x):
    # x is a list (or vector) of length 220
    return max(0, x[77])
def l20_78(x):
    # x is a list (or vector) of length 220
    return max(0, x[78])
def l20_79(x):
    # x is a list (or vector) of length 220
    return max(0, x[79])
def l20_80(x):
    # x is a list (or vector) of length 220
    return max(0, x[80])
def l20_81(x):
    # x is a list (or vector) of length 220
    return max(0, x[81])
def l20_82(x):
    # x is a list (or vector) of length 220
    return max(0, x[82])
def l20_83(x):
    # x is a list (or vector) of length 220
    return max(0, x[83])
def l20_84(x):
    # x is a list (or vector) of length 220
    return max(0, x[84])
def l20_85(x):
    # x is a list (or vector) of length 220
    return max(0, x[85])
def l20_86(x):
    # x is a list (or vector) of length 220
    return max(0, x[86])
def l20_87(x):
    # x is a list (or vector) of length 220
    return max(0, x[87])
def l20_88(x):
    # x is a list (or vector) of length 220
    return max(0, x[88])
def l20_89(x):
    # x is a list (or vector) of length 220
    return max(0, x[89])
def l20_90(x):
    # x is a list (or vector) of length 220
    return max(0, x[90])
def l20_91(x):
    # x is a list (or vector) of length 220
    return max(0, x[91])
def l20_92(x):
    # x is a list (or vector) of length 220
    return max(0, x[92])
def l20_93(x):
    # x is a list (or vector) of length 220
    return max(0, x[93])
def l20_94(x):
    # x is a list (or vector) of length 220
    return max(0, x[94])
def l20_95(x):
    # x is a list (or vector) of length 220
    return max(0, x[95])
def l20_96(x):
    # x is a list (or vector) of length 220
    return max(0, x[96])
def l20_97(x):
    # x is a list (or vector) of length 220
    return max(0, x[97])
def l20_98(x):
    # x is a list (or vector) of length 220
    return max(0, x[98])
def l20_99(x):
    # x is a list (or vector) of length 220
    return max(0, x[99])
def l20_100(x):
    # x is a list (or vector) of length 220
    return max(0, x[100])
def l20_101(x):
    # x is a list (or vector) of length 220
    return max(0, x[101])
def l20_102(x):
    # x is a list (or vector) of length 220
    return max(0, x[102])
def l20_103(x):
    # x is a list (or vector) of length 220
    return max(0, x[103])
def l20_104(x):
    # x is a list (or vector) of length 220
    return max(0, x[104])
def l20_105(x):
    # x is a list (or vector) of length 220
    return max(0, x[105])
def l20_106(x):
    # x is a list (or vector) of length 220
    return max(0, x[106])
def l20_107(x):
    # x is a list (or vector) of length 220
    return max(0, x[107])
def l20_108(x):
    # x is a list (or vector) of length 220
    return max(0, x[108])
def l20_109(x):
    # x is a list (or vector) of length 220
    return max(0, x[109])
def l20_110(x):
    # x is a list (or vector) of length 220
    return max(0, x[110])
def l20_111(x):
    # x is a list (or vector) of length 220
    return max(0, x[111])
def l20_112(x):
    # x is a list (or vector) of length 220
    return max(0, x[112])
def l20_113(x):
    # x is a list (or vector) of length 220
    return max(0, x[113])
def l20_114(x):
    # x is a list (or vector) of length 220
    return max(0, x[114])
def l20_115(x):
    # x is a list (or vector) of length 220
    return max(0, x[115])
def l20_116(x):
    # x is a list (or vector) of length 220
    return max(0, x[116])
def l20_117(x):
    # x is a list (or vector) of length 220
    return max(0, x[117])
def l20_118(x):
    # x is a list (or vector) of length 220
    return max(0, x[118])
def l20_119(x):
    # x is a list (or vector) of length 220
    return max(0, x[119])
def l20_120(x):
    # x is a list (or vector) of length 220
    return max(0, x[120])
def l20_121(x):
    # x is a list (or vector) of length 220
    return max(0, x[121])
def l20_122(x):
    # x is a list (or vector) of length 220
    return max(0, x[122])
def l20_123(x):
    # x is a list (or vector) of length 220
    return max(0, x[123])
def l20_124(x):
    # x is a list (or vector) of length 220
    return max(0, x[124])
def l20_125(x):
    # x is a list (or vector) of length 220
    return max(0, x[125])
def l20_126(x):
    # x is a list (or vector) of length 220
    return max(0, x[126])
def l20_127(x):
    # x is a list (or vector) of length 220
    return max(0, x[127])
def l20_128(x):
    # x is a list (or vector) of length 220
    return max(0, x[128])
def l20_129(x):
    # x is a list (or vector) of length 220
    return max(0, x[129])
def l20_130(x):
    # x is a list (or vector) of length 220
    return max(0, x[130])
def l20_131(x):
    # x is a list (or vector) of length 220
    return max(0, x[131])
def l20_132(x):
    # x is a list (or vector) of length 220
    return max(0, x[132])
def l20_133(x):
    # x is a list (or vector) of length 220
    return max(0, x[133])
def l20_134(x):
    # x is a list (or vector) of length 220
    return max(0, x[134])
def l20_135(x):
    # x is a list (or vector) of length 220
    return max(0, x[135])
def l20_136(x):
    # x is a list (or vector) of length 220
    return max(0, x[136])
def l20_137(x):
    # x is a list (or vector) of length 220
    return max(0, x[137])
def l20_138(x):
    # x is a list (or vector) of length 220
    return max(0, x[138])
def l20_139(x):
    # x is a list (or vector) of length 220
    return max(0, x[139])
def l20_140(x):
    # x is a list (or vector) of length 220
    return max(0, -1.0*x[140] + 1.0)
def l20_141(x):
    # x is a list (or vector) of length 220
    return max(0, -1.0*x[141] + 1.0)
def l20_142(x):
    # x is a list (or vector) of length 220
    return max(0, -1.0*x[142] + 1.0)
def l20_143(x):
    # x is a list (or vector) of length 220
    return max(0, -1.0*x[143] + 1.0)
def l20_144(x):
    # x is a list (or vector) of length 220
    return max(0, x[144] + -16.0*x[148] + 16.0*x[152])
def l20_145(x):
    # x is a list (or vector) of length 220
    return max(0, x[145] + -16.0*x[149] + 16.0*x[153])
def l20_146(x):
    # x is a list (or vector) of length 220
    return max(0, x[146] + -16.0*x[150] + 16.0*x[154])
def l20_147(x):
    # x is a list (or vector) of length 220
    return max(0, x[147] + -16.0*x[151] + 16.0*x[155])
def l20_148(x):
    # x is a list (or vector) of length 220
    return max(0, x[156])
def l20_149(x):
    # x is a list (or vector) of length 220
    return max(0, x[157])
def l20_150(x):
    # x is a list (or vector) of length 220
    return max(0, x[158])
def l20_151(x):
    # x is a list (or vector) of length 220
    return max(0, x[159])
def l20_152(x):
    # x is a list (or vector) of length 220
    return max(0, x[160])
def l20_153(x):
    # x is a list (or vector) of length 220
    return max(0, x[161])
def l20_154(x):
    # x is a list (or vector) of length 220
    return max(0, x[162])
def l20_155(x):
    # x is a list (or vector) of length 220
    return max(0, x[163])
def l20_156(x):
    # x is a list (or vector) of length 220
    return max(0, x[164])
def l20_157(x):
    # x is a list (or vector) of length 220
    return max(0, x[165])
def l20_158(x):
    # x is a list (or vector) of length 220
    return max(0, x[166])
def l20_159(x):
    # x is a list (or vector) of length 220
    return max(0, x[167])
def l20_160(x):
    # x is a list (or vector) of length 220
    return max(0, x[168])
def l20_161(x):
    # x is a list (or vector) of length 220
    return max(0, x[169])
def l20_162(x):
    # x is a list (or vector) of length 220
    return max(0, x[170])
def l20_163(x):
    # x is a list (or vector) of length 220
    return max(0, x[171])
def l20_164(x):
    # x is a list (or vector) of length 220
    return max(0, x[172])
def l20_165(x):
    # x is a list (or vector) of length 220
    return max(0, x[173])
def l20_166(x):
    # x is a list (or vector) of length 220
    return max(0, x[174])
def l20_167(x):
    # x is a list (or vector) of length 220
    return max(0, x[175])
def l20_168(x):
    # x is a list (or vector) of length 220
    return max(0, x[176])
def l20_169(x):
    # x is a list (or vector) of length 220
    return max(0, x[177])
def l20_170(x):
    # x is a list (or vector) of length 220
    return max(0, x[178])
def l20_171(x):
    # x is a list (or vector) of length 220
    return max(0, x[179])
def l20_172(x):
    # x is a list (or vector) of length 220
    return max(0, x[180])
def l20_173(x):
    # x is a list (or vector) of length 220
    return max(0, x[181])
def l20_174(x):
    # x is a list (or vector) of length 220
    return max(0, x[182])
def l20_175(x):
    # x is a list (or vector) of length 220
    return max(0, x[183])
def l20_176(x):
    # x is a list (or vector) of length 220
    return max(0, x[184])
def l20_177(x):
    # x is a list (or vector) of length 220
    return max(0, x[185])
def l20_178(x):
    # x is a list (or vector) of length 220
    return max(0, x[186])
def l20_179(x):
    # x is a list (or vector) of length 220
    return max(0, x[187])
def l20_180(x):
    # x is a list (or vector) of length 220
    return max(0, x[188])
def l20_181(x):
    # x is a list (or vector) of length 220
    return max(0, x[189])
def l20_182(x):
    # x is a list (or vector) of length 220
    return max(0, x[190])
def l20_183(x):
    # x is a list (or vector) of length 220
    return max(0, x[191])
def l20_184(x):
    # x is a list (or vector) of length 220
    return max(0, x[192])
def l20_185(x):
    # x is a list (or vector) of length 220
    return max(0, x[193])
def l20_186(x):
    # x is a list (or vector) of length 220
    return max(0, x[194])
def l20_187(x):
    # x is a list (or vector) of length 220
    return max(0, x[195])
def l20_188(x):
    # x is a list (or vector) of length 220
    return max(0, x[196])
def l20_189(x):
    # x is a list (or vector) of length 220
    return max(0, x[197])
def l20_190(x):
    # x is a list (or vector) of length 220
    return max(0, x[198])
def l20_191(x):
    # x is a list (or vector) of length 220
    return max(0, x[199])
def l20_192(x):
    # x is a list (or vector) of length 220
    return max(0, x[200])
def l20_193(x):
    # x is a list (or vector) of length 220
    return max(0, x[201])
def l20_194(x):
    # x is a list (or vector) of length 220
    return max(0, x[202])
def l20_195(x):
    # x is a list (or vector) of length 220
    return max(0, x[203])
def l20_196(x):
    # x is a list (or vector) of length 220
    return max(0, x[204])
def l20_197(x):
    # x is a list (or vector) of length 220
    return max(0, x[205])
def l20_198(x):
    # x is a list (or vector) of length 220
    return max(0, x[206])
def l20_199(x):
    # x is a list (or vector) of length 220
    return max(0, x[207])
def l20_200(x):
    # x is a list (or vector) of length 220
    return max(0, x[208])
def l20_201(x):
    # x is a list (or vector) of length 220
    return max(0, x[209])
def l20_202(x):
    # x is a list (or vector) of length 220
    return max(0, x[210])
def l20_203(x):
    # x is a list (or vector) of length 220
    return max(0, x[211])
def l20_204(x):
    # x is a list (or vector) of length 220
    return max(0, x[212])
def l20_205(x):
    # x is a list (or vector) of length 220
    return max(0, x[213])
def l20_206(x):
    # x is a list (or vector) of length 220
    return max(0, x[214])
def l20_207(x):
    # x is a list (or vector) of length 220
    return max(0, x[215])
def l20_208(x):
    # x is a list (or vector) of length 220
    return max(0, x[216])
def l20_209(x):
    # x is a list (or vector) of length 220
    return max(0, x[217])
def l20_210(x):
    # x is a list (or vector) of length 220
    return max(0, x[218])
def l20_211(x):
    # x is a list (or vector) of length 220
    return max(0, x[219])
def l20_(x):
    # x is a list (or vector) of length 220
    return [
        l20_0(x),
        l20_1(x),
        l20_2(x),
        l20_3(x),
        l20_4(x),
        l20_5(x),
        l20_6(x),
        l20_7(x),
        l20_8(x),
        l20_9(x),
        l20_10(x),
        l20_11(x),
        l20_12(x),
        l20_13(x),
        l20_14(x),
        l20_15(x),
        l20_16(x),
        l20_17(x),
        l20_18(x),
        l20_19(x),
        l20_20(x),
        l20_21(x),
        l20_22(x),
        l20_23(x),
        l20_24(x),
        l20_25(x),
        l20_26(x),
        l20_27(x),
        l20_28(x),
        l20_29(x),
        l20_30(x),
        l20_31(x),
        l20_32(x),
        l20_33(x),
        l20_34(x),
        l20_35(x),
        l20_36(x),
        l20_37(x),
        l20_38(x),
        l20_39(x),
        l20_40(x),
        l20_41(x),
        l20_42(x),
        l20_43(x),
        l20_44(x),
        l20_45(x),
        l20_46(x),
        l20_47(x),
        l20_48(x),
        l20_49(x),
        l20_50(x),
        l20_51(x),
        l20_52(x),
        l20_53(x),
        l20_54(x),
        l20_55(x),
        l20_56(x),
        l20_57(x),
        l20_58(x),
        l20_59(x),
        l20_60(x),
        l20_61(x),
        l20_62(x),
        l20_63(x),
        l20_64(x),
        l20_65(x),
        l20_66(x),
        l20_67(x),
        l20_68(x),
        l20_69(x),
        l20_70(x),
        l20_71(x),
        l20_72(x),
        l20_73(x),
        l20_74(x),
        l20_75(x),
        l20_76(x),
        l20_77(x),
        l20_78(x),
        l20_79(x),
        l20_80(x),
        l20_81(x),
        l20_82(x),
        l20_83(x),
        l20_84(x),
        l20_85(x),
        l20_86(x),
        l20_87(x),
        l20_88(x),
        l20_89(x),
        l20_90(x),
        l20_91(x),
        l20_92(x),
        l20_93(x),
        l20_94(x),
        l20_95(x),
        l20_96(x),
        l20_97(x),
        l20_98(x),
        l20_99(x),
        l20_100(x),
        l20_101(x),
        l20_102(x),
        l20_103(x),
        l20_104(x),
        l20_105(x),
        l20_106(x),
        l20_107(x),
        l20_108(x),
        l20_109(x),
        l20_110(x),
        l20_111(x),
        l20_112(x),
        l20_113(x),
        l20_114(x),
        l20_115(x),
        l20_116(x),
        l20_117(x),
        l20_118(x),
        l20_119(x),
        l20_120(x),
        l20_121(x),
        l20_122(x),
        l20_123(x),
        l20_124(x),
        l20_125(x),
        l20_126(x),
        l20_127(x),
        l20_128(x),
        l20_129(x),
        l20_130(x),
        l20_131(x),
        l20_132(x),
        l20_133(x),
        l20_134(x),
        l20_135(x),
        l20_136(x),
        l20_137(x),
        l20_138(x),
        l20_139(x),
        l20_140(x),
        l20_141(x),
        l20_142(x),
        l20_143(x),
        l20_144(x),
        l20_145(x),
        l20_146(x),
        l20_147(x),
        l20_148(x),
        l20_149(x),
        l20_150(x),
        l20_151(x),
        l20_152(x),
        l20_153(x),
        l20_154(x),
        l20_155(x),
        l20_156(x),
        l20_157(x),
        l20_158(x),
        l20_159(x),
        l20_160(x),
        l20_161(x),
        l20_162(x),
        l20_163(x),
        l20_164(x),
        l20_165(x),
        l20_166(x),
        l20_167(x),
        l20_168(x),
        l20_169(x),
        l20_170(x),
        l20_171(x),
        l20_172(x),
        l20_173(x),
        l20_174(x),
        l20_175(x),
        l20_176(x),
        l20_177(x),
        l20_178(x),
        l20_179(x),
        l20_180(x),
        l20_181(x),
        l20_182(x),
        l20_183(x),
        l20_184(x),
        l20_185(x),
        l20_186(x),
        l20_187(x),
        l20_188(x),
        l20_189(x),
        l20_190(x),
        l20_191(x),
        l20_192(x),
        l20_193(x),
        l20_194(x),
        l20_195(x),
        l20_196(x),
        l20_197(x),
        l20_198(x),
        l20_199(x),
        l20_200(x),
        l20_201(x),
        l20_202(x),
        l20_203(x),
        l20_204(x),
        l20_205(x),
        l20_206(x),
        l20_207(x),
        l20_208(x),
        l20_209(x),
        l20_210(x),
        l20_211(x),
    ]